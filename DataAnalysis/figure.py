...
def main():
	path = r'/Users/Dijkstraaaaa/Documents/TYPE2/data/320503_266359_49.txt'
	cpu_peak_list = extract_data(path, 142)
	# print cpu_peak_list
	# x = np.arange(0, len(cpu_peak_list), 1)
	x = np.arange(0, 200, 1)
	# plot_day_figure(cpu_peak_list, 29, 'Memory PEAK USAGE')
	plt.plot(x, cpu_peak_list[:200])
	plt.axis([0, 200, min(cpu_peak_list[:200]) * 0.9, max(cpu_peak_list[:200]) * 1.1])
	# plt.plot(x, cpu_peak_list)
	# plt.axis([0, len(cpu_peak_list), min(cpu_peak_list) * 0.9, max(cpu_peak_list) * 1.1])
	# plt.axis([0, len(cpu_peak_list), min(cpu_peak_list) * 0.9, 0.1])
	plt.xlabel('Hours')
